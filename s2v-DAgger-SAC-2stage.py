ALGO_NAME = 's2v-DAgger-AAC-SAC-2stage'
# State-to-Visual DAgger, then use AAC-SAC fine-tune, buffer is kept, critic is from the state expert, actor is from the visual student
...
def parse_args():
    ...
    parser.add_argument("--dagger-steps", type=int, default=300_000)
    parser.add_argument("--dagger-num-steps-per-update", type=float, default=1,
...
class Actor(nn.Module):
    # action mean uses visual input, action std uses state input
    def __init__(self, envs):
        ...
        self.action_mean = make_mlp(256+state_dim, [512, 256, action_dim], last_act=False)
        self.state_backbone = nn.Sequential(
            nn.Linear(np.array(envs.single_observation_space['oracle_state'].shape).prod(), 256),
            nn.ReLU(),
            nn.Linear(256, 256),
            nn.ReLU(),
            nn.Linear(256, 256),
    ...
    def forward(self, obs):
        ...
        log_std = self.fc_logstd(self.state_backbone(obs['oracle_state']))
...
if __name__ == "__main__":
    ...
    actor.state_backbone.load_state_dict({
        p_name: checkpoint[key][f'backbone.{p_name}'] 
        for p_name in actor.state_backbone.state_dict()
    })
    actor.fc_logstd.load_state_dict({
        p_name: checkpoint[key][f'fc_logstd.{p_name}'] 
        for p_name in actor.fc_logstd.state_dict()
    }) # load actor std from expert ckpt
    ...
    rl_num_updates_per_training = int(args.training_freq // args.num_steps_per_update)
    dagger_num_updates_per_training = int(args.training_freq // args.dagger_num_steps_per_update)
    result = defaultdict(list)
    ...
    while global_step < args.total_timesteps:
        ...
        tic = time.time()
        if global_step < args.dagger_steps:
            for local_update in range(dagger_num_updates_per_training):
                data = rb.sample(args.dagger_batch_size)
                pi_mean = actor.get_eval_action(to_tensor(data.observations, device))
                imitation_loss = F.mse_loss(pi_mean, data.observations['expert_action'])
                actor_optimizer.zero_grad()
                imitation_loss.backward()
                actor_optimizer.step()
                print('i_update:', local_update,'loss:', imitation_loss.item())
                if args.bc_loss_th is not None and imitation_loss.item() < args.bc_loss_th:
                    break
        else:
            for local_update in range(rl_num_updates_per_training):
                global_update += 1
        ...
        if (global_step - args.training_freq) // args.log_freq < global_step // args.log_freq:
            if len(result['return']) > 0:
                ...
                result = defaultdict(list)
            if global_step < args.dagger_steps:
                writer.add_scalar("losses/bc_loss", imitation_loss.item(), global_step)
                writer.add_scalar("losses/num_updates", local_update + 1, global_step)
            else:
