...
if __name__ == "__main__":
    ...
    if is_ms1_env(args.env_id):
        LOG_STD_MIN = -20
        assert not args.value_always_bootstrap
    ...
    while global_step < args.total_timesteps:
        ...
        for local_step in range(args.training_freq // args.num_envs):
            ...
            if args.value_always_bootstrap:
                bootstrap_mask = truncations | terminations # if value_always_bootstrap, all dones are regarded as truncations
            else:
                bootstrap_mask = truncations & (~terminations)
            for idx, bootstrap in enumerate(bootstrap_mask):
                if bootstrap:
