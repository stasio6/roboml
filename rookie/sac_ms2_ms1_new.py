...
from rookie.utils.logger import setup_logger
...
def parse_args():
    ...
    parser.add_argument("--env-id", type=str, default="MoveBucket_unified-v1",
        help="the id of the environment")
    parser.add_argument("--obj-id", type=str, default=None)
    parser.add_argument("--link-id", type=int, default=-1)
    ...
    parser.add_argument("--training-freq", type=int, default=32)
    ...
    parser.add_argument("--stop-success-threshold", type=float, default=1.1)
    parser.add_argument("--stop-return-threshold", type=float, default=1e5)
...
def make_env(env_id, seed, control_mode=None, obj_id=None, link_id=-1, video_dir=None, video_trigger=None):
    def thunk():
        assert control_mode is None
        kwargs = {}
        if obj_id is not None:
            kwargs['model_ids'] = [obj_id]
        if link_id >= 0:
            kwargs['fixed_target_link_idx'] = link_id
        env = gym.make(env_id, reward_mode='dense', obs_mode='state', **kwargs)
        if video_dir:
...
if __name__ == "__main__":
    ...
    envs = VecEnv(
        [make_env(args.env_id, args.seed + i, args.control_mode,
                  obj_id=args.obj_id, link_id=args.link_id) 
    ...
    while global_step < args.total_timesteps:
        ...
        if (global_step - args.training_freq) // args.eval_freq < global_step // args.eval_freq:
            tic = time.time()
            eval_result = evaluate(args.num_eval_episodes, actor, eval_envs, device)
            eval_time += time.time() - tic
            for k, v in eval_result.items():
                writer.add_scalar(f"eval/{k}", np.mean(v), global_step)
            mean_suc = np.mean(eval_result['success'])
            mean_ret = np.mean(eval_result['return'])
            logger.info(f'Eval at {global_step}: success {mean_suc:.4f}, return {mean_ret:.4f}')
            if mean_ret > best_return:
                best_return = mean_ret
                save_ckpt('best_eval_return')
                logger.info(f'### Update best return: {mean_ret:.4f}')
            if mean_suc >= args.stop_success_threshold and mean_ret >= args.stop_return_threshold:
                save_ckpt('early_stop')
                break
