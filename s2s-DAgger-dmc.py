...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="s2s-dmc-baselines",
    ...
    parser.add_argument("--num-bc-epochs", type=int, default=2000, # should be tuned based on sim time and tranining time
...
class DMCStateGymWrapper(gym.Wrapper):
    def __init__(self, env, domain, task, action_repeat):
        super().__init__(env)
        obs_dim = int(sum(np.prod(v.shape) for v in env.observation_spec().values()))
        act_shp = env.action_spec().shape
        self.observation_space = gym.spaces.Box(-float("inf"), float("inf"), shape=(obs_dim,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        return self._obs_to_array(time_step.observation), reward, done, info
...
def make_env(env_id, seed, action_repeat=None, video_dir=None, video_trigger=None):
    ...
    def thunk():
        ...
        env = DMCStateGymWrapper(env, domain, task, action_repeat)
...
if __name__ == "__main__":
    ...
    eval_envs = VecEnv(
        [make_env(args.env_id, args.seed + 1000 + i, video_dir=f'{log_path}/videos' if args.capture_video and i == 0 else None) 
    ...
    args.expert_ckpt = '/cephfs_fast/checkpoints_dmc/' + args.env_id + '/checkpoints/' + args.env_id + ".pt"
    #print(args.ex)
