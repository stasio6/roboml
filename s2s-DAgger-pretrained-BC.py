...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="s2s-pretrained",
    ...
    parser.add_argument("--num-steps-per-update", type=float, default=0.5, # should be tuned based on sim time and tranining time
    ...
    parser.add_argument("--bc-loss-th", type=float, default=0.01,
    ...
    parser.add_argument("--init-ckpt", type=str)
...
if __name__ == "__main__":
    ...
    args.init_ckpt = 'checkpoints_bc/' + args.env_id + '/checkpoints/' + args.env_id + ".pt"
    if args.init_ckpt is not None:
        ckpt = torch.load(args.init_ckpt)
        agent.load_state_dict(ckpt['agent'])
        print('## Initilized from ckpt: ', args.init_ckpt)
