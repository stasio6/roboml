...
def parse_args():
    ...
    parser.add_argument("--tgrl-alpha", type=float, default=1)
    parser.add_argument("--gradient-descent-coef", type=float, default=0.01,
        help="tgrl gradient descent coefficient")
    parser.add_argument("--init-lambda", type=float, default=0.1,
        help="tgrl initial lambda")
...
if __name__ == "__main__":
    ...
    while global_step < args.total_timesteps:
        ...
        tic = time.time()
        for used_actor in [actor, actorR]:
            for local_step in range(args.training_freq // args.num_envs // 2):
                ...
                if not learning_has_started:
                    ...
                    actions, _, _ = used_actor.get_action(to_tensor(obs, device))
        ...
        for local_update in range(num_updates_per_training):
            ...
            actorR_optimizer.zero_grad()
            actorR_rl_loss.backward()
            actorR_optimizer.step()
